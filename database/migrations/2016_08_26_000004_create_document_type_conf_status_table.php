...
class CreateDocumentTypeConfStatusTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('document_type_conf_status', function (Blueprint $table) {
            ...
            $table->integer('document_type_conf_id')->unsigned()->comment('Document type configuration reference.');
            $table->integer('document_type_status_id')->unsigned()->comment('Document type state reference.');
            $table->timestamps();
