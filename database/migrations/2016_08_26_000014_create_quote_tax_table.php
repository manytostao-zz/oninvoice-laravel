...
class CreateQuoteTaxTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('quote_tax', function (Blueprint $table) {
            ...
            $table->integer('quote_id')->unsigned()->comment('Quote tax invoice reference.');
            $table->integer('quote_tax_id')->unsigned()->comment('Quote tax tax reference.');
            $table->timestamps();

            $table->foreign('quote_id', 'quote_tax_quote_fk')->references('id')->on('quote')->onDelete('no action')->onUpdate('no action');
        });
    }
